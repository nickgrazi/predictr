...
class Analysis:
    ...
    def __init__(
        ...
        est_type="median",
        unit="-",
        x_label="Time to Failure",
        y_label="Unreliability",
        xy_fontsize=12,
        ...
        **kwargs,
        ...
        bs_size : int, optional
            Sets number of bootstrap samples. The default is 5000.
        ...
        **kwargs :
        ...
        if df is None and ds is None:
            raise ValueError(
                "No data given. Please enter failures \
                             and/or suspensions."
            )
        ...
        if bounds is None and (
            bounds_type != "2s" or bounds_type != "1su" or bounds_type != "1sl"
        ):
            bounds_type = None
    ...
    def mle(self, dist="weibull"):
        ...
        def ll_weib_beta_uncen_2(beta_, df):
            ...
            iterable = (
                ((-1 / beta_**2) - (x / eta) ** beta_ * (np.log(x / eta) ** 2))
                for x in df
            )
        ...
        def ll_weib_beta_cen_2_r(beta_, df, ds):
            ...
            return np.sum(np.fromiter(iterable, float)) + np.sum(
                np.fromiter(iterable2, float)
            )
        ...
        def np_bootstrap(sample, bs_size, est_type="median"):
            ...
            with np.errstate(divide="ignore", invalid="ignore"):
                ...
                if est_type == "median":
                    ...
                    eta_bs = (1 / len(sample) * np.sum(np.fromiter(it, float))) ** (
                        1 / beta_bs
                    )
                elif est_type == "mean":
        ...
        def p_bootstrap(sample, bs_size, est_type="median"):
            ...
            for _ in range(bs_size):
                y = Analysis(
                    df=list(
                        weibull_min.rvs(beta_0, loc=0, scale=eta_0, size=len(sample))
                    )
                )
                y.mle()
        ...
        if dist == "weibull":
            ...
            def beta_init(df, ds=None):
                ...
                sig_0 = np.sum(
                    x_k * (ln_x_k ** (-1)) * (ln_x_k**2 + 0 * ln_x_k + 0)
                ) / np.sum(x_k * (ln_x_k**2 + ln_x_k + 1))
                sig_2 = np.sum(
                    x_k * ln_x_k ** (2 - 1) * (ln_x_k**2 + 2 * ln_x_k + 2)
                ) / np.sum(x_k * (ln_x_k**2 + ln_x_k + 1))
                sig_3 = np.sum(
                    x_k * ln_x_k ** (3 - 1) * (ln_x_k**2 + 3 * ln_x_k + 3)
                ) / np.sum(x_k * (ln_x_k**2 + ln_x_k + 1))
                sig_4 = np.sum(
                    x_k * ln_x_k ** (4 - 1) * (ln_x_k**2 + 4 * ln_x_k + 4)
                ) / np.sum(x_k * (ln_x_k**2 + ln_x_k + 1))
                sig_5 = np.sum(
                    x_k * ln_x_k ** (5 - 1) * (ln_x_k**2 + 5 * ln_x_k + 5)
                ) / np.sum(x_k * (ln_x_k**2 + ln_x_k + 1))
                ...
                beta_ = beta1 * (
                    ...
                    + ((5 * sig_3 * sig_2 / 12) - (5 * sig_2**3 / 8 - sig_4 / 24))
                    * sig_0**4
                    + (
                        (sig_5 / 120)
                        + (7 * sig_2**2 * sig_3 / 8)
            ...
            if self.ds is None:
                ...
                self.eta = (
                    (1 / len(self.df)) * np.sum(np.fromiter(iter_eta, float))
                ) ** (1 / self.beta)
        ...
        if self.bcm is not None:
            if self.bcm == "c4":
                if len(self.df) < 2:
                    raise ValueError(
                        "C4 bias correction requires two or more failures."
                    )
                self.beta_c4 = (
                    self.beta
                    * (
                        np.sqrt(2 / (len(self.df) - 1))
                        * gamma(len(self.df) / 2)
                        / gamma((len(self.df) - 1) / 2)
                    )
                    ** 3.52
                )
                ...
                if self.ds is None:
                    ...
                    self.eta_hrbu = (
                        1 / len(self.df) * np.sum(np.fromiter(it, float))
                    ) ** (1 / self.beta_hrbu)
                else:
                    self.beta_hrbu = self.beta / (
                        1
                        + (
                            1.37
                            / ((len(self.df)) - 1.92)
                            * np.sqrt((len(self.df) + len(self.ds)) / len(self.df))
                        )
                    )
                ...
                if self.ds is None:
                    self.beta_np_bs, self.eta_np_bs = np_bootstrap(
                        self.df, self.bs_size, self.est_type
                    )
                else:
                    raise ValueError(f'"{self.bcm}" does not support suspensions yet')
            elif self.bcm == "p_bs":
                if self.ds is None:
                    self.beta_p_bs, self.eta_p_bs = p_bootstrap(
                        self.df, self.bs_size, self.est_type
                    )
                else:
                    raise ValueError(f'"{self.bcm}" does not support suspensions yet')
    ...
    def median_rank_cens(self, cl=0.5):
        ...
        for i in range(1, len(self.df) + 1):
            adj_ranks.append(
                (rev_rank[i - 1] * prev_rank + n + 1) / (rev_rank[i - 1] + 1)
            )
    ...
    def pb_bounds(self, method_call):
        ...
        if method_call == "mrr":
            for i in range(self.bs_size):
                y = Analysis(
                    df=list(
                        weibull_min.rvs(beta_0, loc=0, scale=eta_0, size=len(self.df))
                    )
                )
                y.mrr()
                df.loc[i] = list(
                ...
                while j < self.bs_size:
                    ...
                    if self.ds is None:
                        try:
                            ...
                            bs_samples = list(
                                np.random.choice(
                                    self.df, size=len(self.df), replace=True
                                )
                            )
                        ...
                        try:
                            ...
                            bs_samples_idx = np.random.choice(
                                len(dat), size=len(dat), replace=True
                            )
    ...
    def mcp_bounds(self):
        ...
        for i in range(self.bs_size):
            y = Analysis(
                df=list(weibull_min.rvs(1.0, loc=0, scale=1.0, size=len(self.df)))
            )
            y.mrr()
            df.loc[i] = (np.log(y.eta) - np.log(np.log(1 / (1 - self.unrel)))) / (
                1 / y.beta
            )
        ...
        if self.bounds_type == "2s":
            ...
            self.bounds_upper = [
                np.exp(np.log(self.eta) - i / self.beta) for i in bounds_lower_z_p
            ]
            self.bounds_lower = [
                np.exp(np.log(self.eta) - i / self.beta) for i in bounds_upper_z_p
            ]
        elif self.bounds_type == "1su":
            lower_perc_position = ceil(self.bs_size * ((1 - self.cl) / 2)) - 1
    ...
    def plot_mrr(self):
        ...
        plt.xlabel(
            f"{self.x_label}{' in ' + self.unit if self.unit != '-' else ''}",
            color="black",
        ...
        plt.semilogx(
            xvals,
            unrel_func(xvals, self.beta, self.eta),
            color="mediumblue",
            linestyle="-",
            linewidth=1.5,
            zorder=2,
        ...
        if self.bounds is not None:
            ...
            if self.bounds_type == "2s":
                if self.bounds == "bbb":
                    yerr_lower = weibull_prob_paper(med_ranks) - weibull_prob_paper(
                        self.bounds_lower
                    )
                    yerr_upper = weibull_prob_paper(
                        self.bounds_upper
                    ) - weibull_prob_paper(med_ranks)
                    plt.errorbar(
                        x=self.df,
                        y=weibull_prob_paper(med_ranks),
                    ...
                    plt.fill_betweenx(
                        y=weibull_prob_paper(self.unrel),
        ...
        if self.plot_ranks:
            if self.ds is None:
                plt.semilogx(
                    self.df,
                    weibull_prob_paper(self.median_rank()),
                    marker="o",
                ...
                plt.semilogx(
                    self.df,
                    weibull_prob_paper(self.median_rank_cens()),
                    marker="o",
                    markerfacecolor="mediumblue",
                    markeredgecolor="mediumblue",
                    markersize=4,
    ...
    def fisher_bounds(self):
        ...
        if self.ds is None:
            element_1 = np.sum(
                [(-1 / b**2) - (x / eta) ** b * (np.log(x / eta) ** 2) for x in self.df]
            )
            element_2 = np.sum(
                [
                    b / (eta**2) - ((x / eta) ** b) * (b / (eta**2)) * (b + 1)
                    for x in self.df
                ]
            )
            element_3 = np.sum(
                [
                    -1 / eta
                    + ((x / eta) ** b) * (1 / eta) * ((b * np.log(x / eta)) + 1)
                    for x in self.df
                ]
            )
            ...
            element_2 = np.sum(
                ...
                [-1 * ((x / eta) ** b) * b / (eta**2) * (b + 1) for x in self.ds]
            )
            element_3 = np.sum(
                [
                    -1 / eta
                ...
                [(x / eta) ** b / eta * (b * np.log(x / eta) + 1) for x in self.ds]
            )
        ...
        var_y = (
            self.f_inv.item(3) / eta**2
            + (np.log(-np.log(1 - np.array(self.unrel)))) ** 2
            * self.f_inv.item(0)
            / b**4
            - (
                2
                * np.log(-np.log(1 - np.array(self.unrel)))
                * self.f_inv.item(1)
                / ((b**2) * eta)
            )
        )
        ...
        if self.bounds_type == "2s":
            y_l = (
                np.log(eta)
                + (np.log(-np.log(1 - np.array(self.unrel))) / b)
    ...
    def lrb(self):
        ...
        def ll_full(beta_, eta, df, ds):
            ...
            return (
                len(df) * (np.log(beta_) - beta_ * np.log(eta))
        ...
        def zerofinder(b_init, eta_init, z):
            ...
            _beta_range = np.linspace(
                min(beta_lrb) - delta_beta, max(beta_lrb) + delta_beta, 400
            )
            ...
            _eta_range = np.linspace(
                min(eta_lrb), max(eta_range_init[_init_sol_eta + 1]), 300
            )
            ...
            if self.ds is None:
                _z = 2 * np.log(
                    np.exp(
                        ...
                        - ll_full_no_cens(
                            np.array([self.sol_b]), np.array([self.sol_eta]), self.df
                        )
                    )
                ) + chi2.ppf(self.cl_lrb, 1)
            else:
                _z = 2 * np.log(
                    np.exp(
        ...
        self.beta_range_init = np.arange(
            0.2 * self.beta_f_range[0], 2 * self.beta_f_range[1], 0.05
        )
        self.eta_range_init = np.linspace(
            0.5 * self.eta_f_range[0], 2 * self.eta_f_range[1], 1000
        )
            ...
            if self.ds is None:
                self.z = 2 * np.log(
                    np.exp(
                        ...
                        - ll_full_no_cens(np.array([b]), np.array([eta]), self.df)
                    )
                ) + chi2.ppf(self.cl_lrb, 1)
                self.beta_lrb, self.eta_lrb = zerofinder(
                    self.beta_range_init, self.eta_range_init, self.z
                )
            else:
                self.z = 2 * np.log(
                    np.exp(
                        ll_full(bb, ee, self.df, self.ds)
                        - ll_full(np.array([b]), np.array([eta]), self.df, self.ds)
                    )
                ) + chi2.ppf(self.cl_lrb, 1)
                self.beta_lrb, self.eta_lrb = zerofinder(
                    self.beta_range_init, self.eta_range_init, self.z
        ...
        self.bounds_lower, self.bounds_upper = t_bounds_from_pars(
            self.beta_lrb, self.eta_lrb, self.unrel
        )
    ...
    def plot(self):
        ...
        if self.bcm == "c4":
            ...
            if self.bounds is None:
                self.tmin_plot = (
                    (-np.log(0.999)) ** (1 / self.beta_hrbu)
                ) * self.eta_hrbu
                self.tmax_plot = (
                    (-np.log(0.001)) ** (1 / self.beta_hrbu)
                ) * self.eta_hrbu
            else:
                if self.bounds_type == "2s":
                    self.tmin_plot = min(self.bounds_lower)
            ...
            if self.bounds is None:
                self.tmin_plot = (
                    (-np.log(0.999)) ** (1 / self.beta_np_bs)
                ) * self.eta_np_bs
                self.tmax_plot = (
                    (-np.log(0.001)) ** (1 / self.beta_np_bs)
                ) * self.eta_np_bs
            else:
                if self.bounds_type == "2s":
                    self.tmin_plot = min(self.bounds_lower)
            ...
            if self.bounds is None:
                self.tmin_plot = (
                    (-np.log(0.999)) ** (1 / self.beta_p_bs)
                ) * self.eta_p_bs
                self.tmax_plot = (
                    (-np.log(0.001)) ** (1 / self.beta_p_bs)
                ) * self.eta_p_bs
            else:
                if self.bounds_type == "2s":
                    self.tmin_plot = min(self.bounds_lower)
        ...
        if self.bcm == "c4":
            # Plot corrected line
            xvals_c4 = list(
                inverse_weibull(np.array([0.001, 0.9999]), self.beta_c4, self.eta_c4)
            )
            plt.semilogx(
                xvals_c4,
                unrel_func(xvals_c4, self.beta_c4, self.eta_c4),
                color="mediumblue",
                linestyle="-",
                linewidth=1.5,
                zorder=2,
            ...
            # Plot biased line
            xvals = list(
                inverse_weibull(np.array([0.001, 0.9999]), self.beta, self.eta)
            )
            plt.semilogx(
                xvals,
                unrel_func(xvals, self.beta, self.eta),
            ...
            xvals_hrbu = list(
                inverse_weibull(
                    np.array([0.001, 0.9999]), self.beta_hrbu, self.eta_hrbu
                )
            )
            plt.semilogx(
                xvals_hrbu,
                unrel_func(xvals_hrbu, self.beta_hrbu, self.eta_hrbu),
                color="mediumblue",
                linestyle="-",
                linewidth=1.5,
                zorder=2,
            ...
            xvals_np_bs = list(
                inverse_weibull(
                    np.array([0.001, 0.9999]), self.beta_np_bs, self.eta_np_bs
                )
            )
            plt.semilogx(
                xvals_np_bs,
                unrel_func(xvals_np_bs, self.beta_np_bs, self.eta_np_bs),
                color="mediumblue",
                linestyle="-",
                linewidth=1.5,
                zorder=2,
            ...
            xvals_p_bs = list(
                inverse_weibull(
                    np.array([0.001, 0.9999]), self.beta_p_bs, self.eta_p_bs
                )
            )
            plt.semilogx(
                xvals_p_bs,
                unrel_func(xvals_p_bs, self.beta_p_bs, self.eta_p_bs),
                color="mediumblue",
                linestyle="-",
                linewidth=1.5,
                zorder=2,
        ...
        **kwargs,
        ...
        if linestyle is not None:
            ...
            if len(self.objects) != len(linestyle):
                raise ValueError(
                    f"Number of linestyles ({len(linestyle)}) is"
        ...
        for key, val in self.objects.items():
            ...
            if (getattr(val, "bounds_lower")) is None and (
                getattr(val, "bounds_upper")
            ) is None:
                if getattr(val, "beta_c4") is not None:
                    dat = list(
                        inverse_weibull(
                            np.array([0.001, 0.9999]),
                            getattr(val, "beta_c4"),
                            getattr(val, "eta_c4"),
                        )
                    )
        ...
        if x_bounds is None:
            ...
            if type(x_bounds) != list:
                raise TypeError(
                    f"x_bounds need to be of type list and not {type(x_bounds)}."
                )
        ...
        plt.xlabel(
            f"{x_label}{' in ' + self.unit if self.unit != '-' else ''}",
            color="black",
            fontsize=xy_fontsize,
        )
        ...
        for key, val in self.objects.items():
            if getattr(val, "bounds") is None:
                ...
                if getattr(val, "beta_c4") is not None:
                    ...
                    plt.semilogx(
                        xplot,
                        unrel_func(
                            xplot, getattr(val, "beta_c4"), getattr(val, "eta_c4")
                        ),
                        color=col,
                        linestyle=ls,
                        linewidth=1.5,
                    ...
                    plt.semilogx(
                        xplot,
                        unrel_func(
                            xplot, getattr(val, "beta_hrbu"), getattr(val, "eta_hrbu")
                        ),
                        color=col,
                        linestyle=ls,
                        linewidth=1.5,
                    ...
                    plt.semilogx(
                        xplot,
                        unrel_func(
                            xplot, getattr(val, "beta_np_bs"), getattr(val, "eta_np_bs")
                        ),
                        color=col,
                        linestyle=ls,
                        linewidth=1.5,
                    ...
                    plt.semilogx(
                        xplot,
                        unrel_func(
                            xplot, getattr(val, "beta_p_bs"), getattr(val, "eta_p_bs")
                        ),
                        color=col,
                        linestyle=ls,
                        linewidth=1.5,
                    ...
                    plt.semilogx(
                        xplot,
                        unrel_func(xplot, getattr(val, "beta"), getattr(val, "eta")),
                        color=col,
                        linestyle=ls,
                        linewidth=1.5,
                        label=f"{key}",
            ...
            if getattr(val, "bounds_type") == "2s":
                if getattr(val, "beta_c4") is not None:
                    ...
                    plt.semilogx(
                        xvals,
                        np.log(-np.log(1 - np.array([0.001, 0.9999]))),
                        color=col,
                        linestyle=ls,
                        linewidth=1.5,
                        zorder=2,
            ...
            if plot_ranks:
                if getattr(val, "ds") is None:
                    ...
                    plt.semilogx(
                        getattr(val, "df"),
                        weibull_prob_paper(
                            median_rank_cens(getattr(val, "df"), getattr(val, "ds"))
                        ),
                        marker="o",
                        markerfacecolor=col,
                        markeredgecolor="black",
        ...
        **kwargs,
        ...
        if style == "scatter":
            for key, val in self.objects.items():
                ...
                plt.scatter(
                    beta, eta, s=3, c=next(color), label=f"{key}: {conf_level * 100}%"
                )
            ...
            for key, val in self.objects.items():
                ...
                eta = getattr(val, "eta_lrb")
                beta_sorted = (
                    list(beta[0::2]) + list(beta[1::2][::-1]) + list(beta[0:1])
                )
    ...
    def weibull_pdf(
        ...
        **kwargs :
        ...
        def wei_pdf(x, beta, eta):
            ...
            """
            return (
                beta / eta * (x / eta) ** (beta - 1) * np.exp(-1 * ((x / eta) ** beta))
            )
        ...
        if len(linestyle) != len(beta) or len(linestyle) != len(eta):
            raise ValueError(
                "Number of linestyles must match the list length of beta and eta."
            )
        ...
        if type(beta) == list and type(eta) == list:
            if labels is not None:
                ...
                for i, j, line in zip(beta, eta, linestyle):
                    plt.plot(
                        xvals, wei_pdf(xvals, i, j), linestyle=line, color=next(color)
                    )
        plt.tight_layout()
